...
- A live development branch (for example the skills dev branch) gets a persistent worktree named `<repo>-<branch>` under the conventional root, so the primary checkout stays on the default branch. Git then refuses to check that branch out in the primary checkout, which closes the loophole of accidentally working the dev branch in the main checkout.
...
- Immediately before any `lake build`, verify that this worktree's cached artifacts still match its current toolchain and manifest; rehydrate if either changed or cache state is uncertain.
- A yielded or backgrounded validation command is still running, not passing. Follow it through the top-level process exit and read every required final sentinel (for example `LINT-ENV: PASS`); partial output from an earlier command in an `&&` chain is not evidence that later checks ran.
